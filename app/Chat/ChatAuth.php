...
class ChatAuth
{
    ...
    public function pusherAuth($channelName, $socket_id, $data = null){
			// echo "<script>console.log('Debug Objects: " . $channelName . "' );</script>";
    ...
    public function deleteConversation($user_id){
        try {
            foreach ($this->fetchMessagesQuery($user_id)->get() as $msg) {
                ...
                if (isset($msg->attachment)) {
                    $path = storage_path('app/public/'.config('chat.attachments.folder').'/'.json_decode( $msg->attachment)->new_name);
                    if(File::exists($path)){
                        File::delete($path);
                    }
                }
